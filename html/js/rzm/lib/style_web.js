var excptMnuObj = {
		"RZMCSCTAP01": "RZMZKMZPC01",
		"RZMCSCTAP02": "RZMZKMZPC01",
		"RZMMYZSUI41": "RZMZKMZPC01",
		"RZMPCSCCM99": "RZMPCSCSM01",
		"RZMPCSCCM06": "RZMPCSCSM01",
		"RZMOCCCCR02": "RZMOCCCCR01",
		"RZMOCCCRC02": "RZMOCCCRC01",
		"RZM2MYLON11": "RZM2MYLON10",
		"RZMCPSCPR01": "RZMCPSCPR00",
		"RZMPCSLCM07": "RZMPCSBCM01",
		"RZMMYZMYZ01": "RZMMYZSUI10"
};

function setMySelectMnu() {
	var selectedMnuId =  BASE_MENU_ID;

	if(selectedMnuId in excptMnuObj){
		selectedMnuId = excptMnuObj[selectedMnuId];
	}
	
	// 현재 위치 선택
	var findMnuObj =  $('.gnbList').find("a[data-mnu-id^='"+selectedMnuId.substring(0, 9)+"']") ;
	if(findMnuObj.length > 0) {
		
		if($(findMnuObj).closest(".depth02").length > 1) {
			findMnuObj =  $('.gnbList').find("a[data-mnu-id='"+selectedMnuId+"']");
			if(findMnuObj.length == 0) {
				$('.gnbList a', $('#gnb')).removeClass('on').next().hide();
				$('.depth01 > li:first-child > a', $('#gnb')).addClass('on').next().show();
		...
		$('.gnbList a', $('#gnb')).removeClass('on').next().hide();
		$(findMnuObj).closest(".depth02").css("display", "block");
		$(findMnuObj).closest(".depth02").prev().addClass("on");
		
		setClosetObjShow(findMnuObj, ".depth03", selectedMnuId);
		setClosetObjShow(findMnuObj, ".depth04", selectedMnuId);
	}
}
function setClosetObjShow(findMnuObj, classId, selectedMnuId) {
	
	if($(findMnuObj).closest(classId).length > 0) {
		if($(findMnuObj).closest(classId).length > 1) {
			findMnuObj = $('.gnbList').find("a[data-mnu-id='"+selectedMnuId+"']") ;
		}
		$(findMnuObj).closest(classId).css("display", "block");
		$(findMnuObj).closest(classId).prev().addClass("on");
	}
}
/********************************************************************************************
** $(document).ready
...
/* 2018-06-22 */
	...
	$('#gnb .gnbClose').on('click', function(e) {//전체메뉴 닫기
		...
		// 현재 위치 선택
		var findMnuObj =  $('.gnbList').find("a[data-mnu-id^='"+BASE_MENU_ID.substring(0, 9)+"']") ;
		if(findMnuObj.length == 0) {
