...
function tbl_board(){
	jQuery('.tbl-board .tit a').bind('click', function(){
		if(jQuery(this).parent().parent().next().css('display') == 'none'){
			...
			jQuery(this).closest('tr').next('.detailView').attr('tabIndex', '0').focus();
		}else{
...
$(function() {
    ...
    btnAll.click(function() {
	  ...
	  $('.allMenuCon .fc > a').attr("tabIndex", "0").focus();
    });
    ...
    closeAllMenu.click(function() {
      ...
      gnbDim.hide();
      event.preventDefault();
	  $(".allMenuBtn a").attr("tabIndex", "0").focus();
    });
...
function locationMenu(){
	...
	var rnewInqMnuArr = ['2CS','2CC','2PS','2RS','3CS','2RE','2CI'];
	var orgPrtctMnuArr = ['CDS','PNS','PSS','VVV','SSS','FPS','WSS','PRT'];
	var rnewPrtctMnuArr = ['2DS','2NS','2PC','2VV','2SS','3PS','2WS','2RT'];
	...
	else if (strMENU_ID == "PCS"||strMENU_ID == "FCS"||strMENU_ID == "CRS"||strMENU_ID == "INQ"||$.inArray(strMENU_ID , rnewInqMnuArr) > -1)
        $(".menu:eq(2)").addClass('on');
	else if ($.inArray(strMENU_ID , orgPrtctMnuArr) > -1||$.inArray(strMENU_ID , rnewPrtctMnuArr) > -1)
...
$(document).ready(function() {
    ...
    // 포커스 아웃될때 depth3 닫기
    var depthLast  = $('#snb .depth3').last().find('li a').last();
	depthLast.focusout(function(){
			$(this).closest('.depth3').closest('li').removeClass('active');
	});
...
//	appendHidden(paramForm, 'userNm',  $("#user_name").val());
	...
	callAjax(form, "BZWOCCNSE05002AM",function(data){
		...
		if(data.RESULT_CD =="00" ){
			...
			changeMenu(paramForm, "BZW2CCNCS00", "BZW2CCNCS05001VM", "BZW2CCNCS05001VM");
			callMenu(paramForm);
		}else{
			alert("유효한 보고서가 없습니다.");
			return;
